<?php
/* ─────────────────────────────────────────────
   Contact / "Request a Demo" handler.
   Receives the contact-page form and emails it to $callback_to via
   Hostinger SMTP using PHPMailer. Returns JSON. Mirrors send-callback.php.
───────────────────────────────────────────── */
...
$clean = static function (string $v, int $max = 160): string {
  $v = trim($v);
  $v = preg_replace('/[\r\n\t]+/', ' ', $v);
...
$students = $clean($_POST['students'] ?? '', 40);
...
$html = '<div style="font-family:Arial,Helvetica,sans-serif;max-width:520px">'
      . '<h2 style="margin:0 0 4px;color:#4B5D3F;font-size:18px">New demo request</h2>'
      . '<p style="margin:0 0 16px;color:#5a6552;font-size:13px">From the Schoozie contact page</p>'
      . '<table style="border-collapse:collapse">' . $htmlRows . '</table></div>';
$text = "New demo request (Schoozie contact page)\n\n" . $textRows;
...
try {
  ...
  $mail->Subject = 'New Demo Request — ' . $name;
  $mail->isHTML(true);
  $mail->Body    = $html;
  $mail->AltBody = $text;
  ...
  error_log('[send-contact] ' . $mail->ErrorInfo);
